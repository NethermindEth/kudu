...
library WeightedOracleLibrary {
    ...
    function consult(address pool, uint32 period) internal view returns (PeriodObservation memory observation) {
        ...
        observation.arithmeticMeanTick = int24(tickCumulativesDelta / int56(uint56(period)));
        // Always round to negative infinity
        if (tickCumulativesDelta < 0 && (tickCumulativesDelta % int56(uint56(period)) != 0)) observation.arithmeticMeanTick--;
        ...
        returns (int24 arithmeticMeanWeightedTick)
    {
        ...
        for (uint256 i; i < observations.length; i++) {
            numerator += int256(uint256(observations[i].harmonicMeanLiquidity)) * observations[i].arithmeticMeanTick;
