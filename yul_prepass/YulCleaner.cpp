...
YulCleaner::YulCleaner(std::string sol_src, std::string main_contract,
...
bool isFunctionSig(std::string line) {
  if (line.find("function ") != std::string::npos and
...
std::vector<std::string> YulCleaner::removePreamble(
...
std::vector<std::string> YulCleaner::getRuntimeYul(
...
FinalizedYul YulCleaner::removeDeploymentCode(std::vector<std::string> code) {
  ...
  auto onlyDefinitions =
      std::vector<std::string>(code.begin() + start, code.end());
  auto entrySequence =
      std::vector<std::string>(code.begin() + 3, code.begin() + start - 2);
...
std::vector<std::string> YulCleaner::getEndOfOjbect(
    std::vector<std::string> lines) {
  int end = 0;
  for (std::size_t i = 0; i < lines.size(); i++) {
    std::string lineCopy = lines[i];
...
std::string YulCleaner::removeNonDynamicDispatch(
    std::vector<std::string> entrySeq) {
  auto newEntrySeq = concatCaseBlocks(entrySeq);
  std::string prefix;
  std::find_if(entrySeq.begin(), entrySeq.end(),
  ...
  for (size_t i = 0; i < newEntrySeq.size(); i++) {
    if (newEntrySeq[i].find("case") != std::string::npos) {
      auto selectorInEntrySeq =
          newEntrySeq[i].substr(newEntrySeq[i].find("0x") + 2, 8);
  ...
  if (entryStr.find("case") == std::string::npos) return "";
  return entryStr;
}

std::vector<std::string> YulCleaner::concatCaseBlocks(
    std::vector<std::string> entrySeq) {
...
std::pair<std::string, int> YulCleaner::endOfCaseBlock(
    std::vector<std::string> caseBlock, int startPos) {
  for (size_t i = startPos; i < caseBlock.size(); i++) {
    if (caseBlock[i].find("case") != std::string::npos) {
      ...
      std::for_each(block.begin(), block.end(),
                    [&strRepr](std::string line) { strRepr += line + "\n"; });
      return std::pair<std::string, int>(strRepr, i - startPos + 1);
    } else if (caseBlock[i].find("return") != std::string::npos and
               caseBlock[i + 2].find("case") == std::string::npos) {
      auto block = std::vector<std::string>(caseBlock.begin() + startPos,
                                            caseBlock.begin() + i + 2);
      std::string strRepr;
      std::for_each(block.begin(), block.end(),
  ...
  for (auto contract : contracts) {
    for (auto hash : contract["hashes"].items()) {
      ...
      if (not contains_warp(this->m_storageVars, name)) {
        this->m_publicFunctionSelectors.functionNames.emplace_back(name);
        this->m_publicFunctionSelectors.selectors.emplace_back(hash.value());
      }
    }
  }
}
...
bool YulCleaner::isExtCodeSizeCheck(std::array<std::string, 6> lines) {
  return (lines[0].find("extcodesize") != std::string::npos and
          lines[1].find("iszero") != std::string::npos and
          lines[2].find("if") != std::string::npos and
          lines[3].find("{") != std::string::npos and
          lines[4].find("revert") != std::string::npos and
...
std::vector<std::string> YulCleaner::removeExtCodeSizeCheck(
  ...
  for (size_t i = 0; i < yul.size(); i += increment) {
    if (i + 6 < yul.size()) {
      std::array<std::string, 6> seq = {yul[i],     yul[i + 1], yul[i + 2],
                                        yul[i + 3], yul[i + 4], yul[i + 5]};
