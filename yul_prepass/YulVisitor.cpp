...
YulVisitor::YulVisitor(std::string src,
					   std::string warpConstructorSelector,
					   std::string warpConstructorName)
{
	m_src					   = src;
	m_constructorSelector	   = warpConstructorSelector;
	m_constructorName		   = warpConstructorName;
	m_dynamicArgsInConstructor = (warpConstructorName.find("_DynArgs")
								  != std::string::npos);
...
void YulVisitor::operator()(const yul::VariableDeclaration& _node)
{
	...
	{
		auto declr	= std::string(m_src.begin() + _node.debugData->location.start,
		...
		if (declr.find(":= and(") != std::string::npos
			and m_prevDeclr.find(":= sub(shl(160, 1), 1)") != std::string::npos)
		{
			std::cout << "HERE" << std::endl;
			if (_node.value)
			{
				const yul::FunctionCall funcCall = std::get<yul::FunctionCall>(std::move(*(_node.value)));
				try 
				{
					const yul::Identifier ident = std::get<yul::Identifier>(funcCall.arguments[0]);
					auto assignStr = _node.variables[0].name.str();
					auto assignStrClean = std::string(assignStr.begin(), assignStr.begin() + assignStr.find_last_of('_'));
					std::cout << ident.name.str() << std::endl;
					std::string newDeclr = "let " +  assignStrClean + " := " + ident.name.str();
					std::cout << newDeclr << std::endl;
					m_src.erase(m_src.begin() + m_prevDeclrLocation[0], m_src.begin() + m_currDeclrLocation[1]);
...
void YulVisitor::operator()(const yul::FunctionDefinition& _node)
{
	...
	if (m_currentFunction == m_constructorName
		and m_currentPass == PassType::Constructor
		and m_dynamicArgsInConstructor)
	{
		std::string constructorPrelude = dynamicEntrySeq + "\n"
										 + m_deletedCase.value();
...
void YulVisitor::operator()(const yul::Case& _node)
{
	if (_node.value)
	{
		if (_node.value->value.str() == m_constructorSelector
			and m_currentPass == PassType::CaseRemoval)
		{
